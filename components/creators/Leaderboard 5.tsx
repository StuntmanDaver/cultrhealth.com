...
export function Leaderboard({ myName }: { myName: string }) {
  ...
  useEffect(() => {
    setLoading(true)
    fetch(`/api/creators/leaderboard?metric=${metric}`)
  ...
  return (
    <div className="bg-white border border-stone-200 rounded-2xl p-6">
      ...
      {loading ? (
        ...
        <div className="py-8 text-center text-sm text-cultr-textMuted">
          ...
          {entries.map((entry) => (
            ...
            >
              ...
              <span className={`flex-1 text-sm ${entry.isYou ? 'font-bold text-cultr-forest' : 'text-cultr-text'}`}>
                {entry.isYou ? (myName?.split(' ')[0] || entry.name) : entry.name}{' '}
                {entry.isYou && <span className="text-xs font-normal text-cultr-textMuted">(you)</span>}
              </span>
              <span className={`text-sm font-medium ${entry.isYou ? 'text-cultr-forest' : 'text-cultr-textMuted'}`}>
                {activeOption.format(entry[metric])}
              </span>
            </div>
          ))}
        </div>
