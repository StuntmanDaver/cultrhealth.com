...
### Added
- **`<VisitorTracker />`** (`components/site/VisitorTracker.tsx`): client component mounted in root `app/layout.tsx` that fires once per browser (guarded by `cultr_first_visit` cookie). Posts `{ site, landingPage, referrer, userAgent }` to `/api/visitor/new`.
- **`/api/visitor/new`** (`app/api/visitor/new/route.ts`): server-side first-visit notification endpoint. Sets the `cultr_first_visit` cookie unconditionally (silences future hits even on rate-limit), then sends a branded notification email to `TEAM_EMAILS` if `formLimiter` (5/hour/IP, prefix `form:visitor-new:*`) allows. All user-supplied fields HTML-escaped.
- **Creator branch in `/api/club/signup`** (`app/api/club/signup/route.ts`): when `signupType === 'creator'`, `handleCreatorFlow()` upserts the `creators` row, resets paused/rejected applicants back to `pending`, and routes email by effective status — portal login link for `active`, "under review" notice for already-verified pending, verify-email link otherwise. Owner notifications fire only for genuinely new applications or resubmissions (skips duplicate pending).
- **Owner notification on `/api/creators/apply`** (`app/api/creators/apply/route.ts`): `sendOwnerCreatorApplicationNotification()` emails owners on every application with auto-approved vs. pending review framing, plus distinct `🔁 Creator re-applied` subject for rejected/paused resubmissions.
- **Recruiter attribution in join signup** (`app/api/club/signup/route.ts`): new `resolveRecruiterIdForSignup()` reads the `cultr_attribution` cookie set by `/r/[slug]` click tracking, validates the recruiter is `active`, blocks self-referrals (`recruiter.email === signupEmail`), and passes `recruiter_id` into `createCreator`. Failure modes (missing cookie, expired, malformed, inactive recruiter) all silently return `undefined` so signup proceeds without attribution rather than 500ing.
- **`setCreatorRecruiterIfMissing(creatorId, recruiterId)` helper** (`lib/creators/db.ts`): backfills `recruiter_id` for an existing creator who was originally created without attribution. Race-safe via `WHERE id = X AND recruiter_id IS NULL` — if a concurrent request set `recruiter_id` between our SELECT and UPDATE, the UPDATE is a no-op and first-touch attribution is preserved. Wired into both `/api/club/signup` (creator branch) and `/api/creators/apply` so apply→later-join AND join→later-apply flows both backfill missed attribution. Backfill calls are wrapped in their own try/catch in both routes — a backfill failure must never block the verify email, owner notification, or 500 the user's resubmission.
- **Shared `CREATOR_NOTIFICATION_EMAILS` constant** (`lib/config/creator-notification-emails.ts`): single source of truth for the "new creator signup/application" notification recipient list (`david, erik, alex, stewart @cultrhealth.com`). Distinct from `lib/config/owner-emails.ts` `OWNER_EMAILS` which filters analytics aggregates (Stewart excluded there because he operates as a real creator; Tony included for analytics, excluded from notifications). Both `/api/club/signup` and `/api/creators/apply` now import from this shared module.

### Fixed
- **Welcome-email link broke the on-domain hard rule** (`app/api/club/signup/route.ts`): `sendClubWelcomeEmail` "Browse Therapies" CTA pointed at `https://cultrclub.com`, violating the "Never link cultrhealth.com → cultrclub.com" rule. Repointed to `https://cultrhealth.com/pricing`.
- **`updateCreatorStatus` was nuking approval history** (`lib/creators/db.ts`): the function unconditionally set `approved_at = NULL` and `approved_by = NULL` whenever status ≠ `'active'`. A creator who was approved (March 1) → paused (March 15) → re-activated (May 1) lost their original approval timestamp on every status change. Now uses CASE preservation: `approved_at` is only set on the FIRST transition to `'active'` and never erased; `approved_by` only updates when an explicit approver is passed. All 5 callers (`club/signup`, `apply`, `admin/.../reject`, `verify-login` staging auto-create) traced and verified safe with the new logic.
- **Stuck rejected/paused status on join resubmission** (`app/api/club/signup/route.ts`): a rejected/paused creator re-applying via the join form silently stayed in that status (verify-email then redirected them to `inactive_account`). Now resets to `pending`, matching `/api/creators/apply`'s existing `shouldResubmitForReview` behavior. Owners receive a `🔁 Creator re-applied` notification for the resubmission.
- **Owner notification spam on duplicate join submissions** (`app/api/club/signup/route.ts`): a pending creator filling out the join form repeatedly fired duplicate `🧑‍🎤 New creator signup` emails to all 4 owners. Added `isNewToOwners = !existing || wasResubmittedForReview` gate so owners only receive notifications for genuinely new applications or rejected/paused resubmissions.
- **Pointless verify email for already-verified pending creators** (`app/api/club/signup/route.ts`): a creator who already verified their email and was awaiting admin review got another "Verify Your Email" link on every join form re-submission. Now sends a "Your CULTR Creator Application Is Under Review" email instead.
- **Active creator received wrong email** (`app/api/club/signup/route.ts`): an already-`active` creator filling out the join form as `signupType=creator` got a "Thanks for applying, our team will review within 48 hours" email. Now branches on `creator.status === 'active'` and sends a portal login link via `/api/creators/verify-login`.
- **`createCreator` skipped when RESEND_API_KEY missing** (`app/api/club/signup/route.ts`): the `if (!process.env.RESEND_API_KEY) return` guard was placed BEFORE `createCreator()`, so missing email config silently aborted the DB write entirely. Guard moved after the upsert.

### Changed
- **`CREATOR_OWNER_EMAILS` constant deduplicated**: was previously inlined in BOTH `app/api/club/signup/route.ts` AND `app/api/creators/apply/route.ts` — silent drift risk if owners changed. Both files now import from the new `lib/config/creator-notification-emails.ts`.
