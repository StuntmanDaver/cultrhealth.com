...
// Logo: Official PNG from public/cultr-health-logo.png (cream variant for front side)
...
// ── Load cream logo PNG as data URI for @react-pdf/renderer ──
import { readFileSync } from 'fs'

function loadLogoPngDataUri() {
  const pngBuffer = readFileSync(join(process.cwd(), 'public/images/email-logo-cream.png'))
  return `data:image/png;base64,${pngBuffer.toString('base64')}`
}
...
const styles = StyleSheet.create({
  ...
  logoImage: {
    width: CARD_W * 0.75,
    // aspect ratio 1563:791 (~1.976:1) from official PNG
    height: (CARD_W * 0.75) / (1563 / 791),
  },
  divider: {
...
async function generateCard() {
  // Load cream logo PNG (generated by scripts/generate-cream-logo.mjs)
  const creamLogoPng = loadLogoPngDataUri()
