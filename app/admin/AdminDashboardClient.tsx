              ...
              {data.allMemberships && data.allMemberships.length > 0 ? (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-brand-primary/10">
                        <th className="text-left py-3 px-4 text-brand-primary/60 font-medium text-sm">Customer ID</th>
                    ...
                    <tbody>
                      {data.allMemberships
                        .filter(m => {
                          ...
                          if (memberSearch && !m.stripe_customer_id.toLowerCase().includes(memberSearch.toLowerCase())) return false
                          return true
                        })
                        .map((m, i) => {
                          ...
                          const isLoading = memberActionLoading === m.stripe_customer_id
                          return (
                            <tr key={m.id} className={i % 2 === 0 ? 'bg-brand-cream/30' : ''}>
                              <td className="py-3 px-4 text-sm font-mono text-brand-primary">
                                <span title={m.stripe_customer_id}>
                                  {m.stripe_customer_id.length > 20 ? `${m.stripe_customer_id.slice(0, 20)}...` : m.stripe_customer_id}
                                </span>
                              </td>
                                  ...
                                  {isActive && (
                                    <>
                                      <button
                                        onClick={() => {
                                          clearMemberActionMessages()
                                          setPauseTarget({ customerId: m.stripe_customer_id, email: m.stripe_customer_id, name: m.plan_tier })
                                          const d = new Date()
                                          d.setDate(d.getDate() + 30)
                                      ...
                                      <button
                                        onClick={() => {
                                          clearMemberActionMessages()
                                          setUpgradeTarget({ customerId: m.stripe_customer_id, email: m.stripe_customer_id, name: m.plan_tier, currentTier: m.plan_tier })
                                          setUpgradeTier('')
                                        }}
                                        disabled={isLoading}
                                      ...
                                      <button
                                        onClick={() => {
                                          clearMemberActionMessages()
                                          setCancelTarget({ customerId: m.stripe_customer_id, email: m.stripe_customer_id, name: m.plan_tier })
                                          setCancelReason('')
                                        }}
                                        disabled={isLoading}
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setPauseTarget(null)}>
            <div className="bg-white rounded-xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
              ...
              <p className="text-sm text-brand-primary/60 mb-4">
                Pausing subscription for <span className="font-mono font-medium text-brand-primary">{pauseTarget.customerId}</span>.
            ...
            <div className="bg-white rounded-xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
                ...
                <p className="text-xs text-red-600 mt-1">
                  This will immediately cancel the Stripe subscription for <span className="font-mono font-medium">{cancelTarget.customerId}</span>. This action cannot be undone.
                </p>
              </div>
              <div className="space-y-4">
                <div>
        ...
        {upgradeTarget && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setUpgradeTarget(null)}>
            <div className="bg-white rounded-xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
              ...
              <p className="text-sm text-brand-primary/60 mb-4">
                Current tier: <span className="font-medium text-brand-primary capitalize">{upgradeTarget.currentTier}</span> for <span className="font-mono text-brand-primary">{upgradeTarget.customerId}</span>.
                  ...
                  >
                    ...
                    {['core', 'catalyst', 'concierge'].filter(t => t !== upgradeTarget.currentTier).map(t => (
                      <option key={t} value={t}>
                        {t === 'core' ? 'Core ($199/mo)' : t === 'catalyst' ? 'Catalyst+ ($499/mo)' : 'Curated ($1,099/mo)'}
                      </option>
                    ))}
                  </select>
                </div>
