...
import { acknowledgeKlarnaOrder, getKlarnaOrder } from '@/lib/payments/klarna-api';
import { updateOrderByOrderNumber, getOrderByOrderNumber } from '@/lib/db';

/**
 * Klarna Webhook Handler
 *
 * Verification strategy: Klarna doesn't provide webhook signatures.
 * Instead, we verify each webhook by calling back to the Klarna Order
 * Management API to confirm the order exists before processing.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Verify the order exists via Klarna API callback
    if (order_id) {
      try {
        await getKlarnaOrder(order_id);
        console.log('Klarna webhook verified via API callback:', order_id);
      } catch (verifyError) {
        console.error('Klarna webhook verification failed - order not found:', {
          order_id,
          error: verifyError,
        });
      ...
      console.error('Klarna webhook missing order_id');
