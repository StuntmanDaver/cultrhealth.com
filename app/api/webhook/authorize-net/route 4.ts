...
async function handleFraudHeld(payload: AuthorizeNetWebhookEvent['payload']) {
  ...
  try {
    const { sendFraudHeldAdminNotification } = await import('@/lib/resend');
    await sendFraudHeldAdminNotification({
      transactionId: payload.id,
      merchantRefId: payload.merchantReferenceId,
      provider: 'Authorize.net',
    });
    console.log('Fraud review admin notification sent');
  } catch (error) {
    console.error('Failed to send fraud review notification:', error);
  }
}
...
async function handleSubscriptionExpiring(payload: AuthorizeNetWebhookEvent['payload']) {
  ...
  try {
    const { getMembershipBySubscriptionId } = await import('@/lib/db');
    const membership = await getMembershipBySubscriptionId(`authnet_${payload.id}`);

    if (membership?.stripe_customer_id) {
      const { sendSubscriptionExpiringEmail } = await import('@/lib/resend');
      // stripe_customer_id stores the email for Authorize.net memberships
      await sendSubscriptionExpiringEmail({
        name: 'there',
        email: membership.stripe_customer_id,
      });
      console.log('Subscription expiring notification sent');
    } else {
      console.log('No membership found for subscription, cannot send expiring notification');
    }
  } catch (error) {
