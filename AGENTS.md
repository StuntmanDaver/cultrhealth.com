...
- **Ghost Session Prevention:** When manually constructing `Set-Cookie` headers via `response.headers.append('Set-Cookie', ...)` in Next.js `NextResponse`, NEVER use `Expires` strings. `Expires` dates contain commas, which break Vercel Edge's header merging logic and cause silent browser parsing failures resulting in infinite `session_timeout` redirect loops. Always use `Max-Age=0` to safely clear cookies.
